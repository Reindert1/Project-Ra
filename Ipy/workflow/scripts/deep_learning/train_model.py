...
def train_model(data_path, model_save_path, model=None,
    ...
    history = model.fit(x_train, y_train, epochs=snakemake.config["max_epochs"], validation_split=0.25,
...
if __name__ == '__main__':
    ...
    data = np.load(input_path, allow_pickle=True, mmap_mode="r")
    input_shape = data.shape[1] - 1

    # Create model
    model: tf.keras.Sequential = create_model((input_shape,), metrics=METRICS)
